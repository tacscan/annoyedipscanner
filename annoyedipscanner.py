...
parser.add_argument('-g', help='Port set to scan, default is nmap default eleven', choices=['1000', 'fast', 'file'], required=False)
            ...
            if fileType == 'c':
                try:
                    ...
                    for x in range(0, len(temp)):
                        if temp[x].strip().isdigit():
                            if (int(temp[x]) > 0) and (int(temp[x]) < 65535):
                                pts.append(int(temp[x].strip()))
                    print('The ports to be tested are:')
                    print(pts)
                    f.close()
                except OSError:
                    ...
                    with open(portFile) as file:
                        while line := file.readline():
                            if line.strip().isdigit():
                                if (int(line.strip()) > 0) and (int(line.strip()) < 65535):
                                    pts.append(int(line.strip()))
                    print('The ports to be tested are:')
                    print(pts)
                except OSError:
                    print("Could not open/read file:", portFile)
