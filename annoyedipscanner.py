...
if __name__ == "__main__":
    ...
    if args['p'] == False:
        ...
        if args['g'] == 'seq':
            print('[+]Testing sequential ports from ' + args['f'] + ' to ' + args['l'] + ' inclusive')
    ...
    if (int(compIndexFullScan)==0):
        ...
        start = time.time()
        print('\nDoing full scan on ' + ipFree[int(compIndexFullScan)-1])
        portsToScan = range(1,65535)
        with concurrent.futures.ThreadPoolExecutor(max_workers=200) as executor:
            future_to_port = {executor.submit(scanPort, ipFree[int(compIndexFullScan)-1], portToScan, portFree, portFound): portToScan for portToScan in portsToScan}
